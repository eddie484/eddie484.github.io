...
타입체크 파트에서는 func_declr인 경우, sem_symbol의 ~~son이 있는 경우를 함수 요소로~~ 타입이 함수인 경우들을 검토한다.

────────────────────────────────

구현 후 테스트해보니, 함수 선언의 인자 처리에 문제가 발생했음.
...
형식으로 되어있어, IDENT를 변수/함수 사용으로 인식해
등록되지 않은 IDENT 호출 시도로 여기게 된다.

해결?

① parser를 고쳐 var_declr로 감싼다?
   -> 초기화 없는 변수 취급일 테니 심볼 등록 후 사라질 것이다.
   -> 타입체킹에 문제 발생, 스코프 제한을 위해 블록 추가해야 함, 사라지지 않게 고쳐야 함.
   -> 문제가 많다!

② 를 만나면, push → IDENT 등록 ~~->pop을~~ 진행,
   를 나올때 pop 하게 하기? 시도해 봐야지.

의 brother는 block이면...
① 거기에 변수 선언 추가하기
③ param_list의 pop을 block까지 out할 때 하기.

함수 사용에선 문제없음: 이미 선언된 변수/함수만이 인자로 올 수 있기 때문.
...
이면, 타입이 brother로 연결되면 어디까지가 인자 하나인지?
~~-> ident 노드를 찾아서 사용하기. 어차피 다른 노드로 감싸도 ident는 찾아야 함.~~
-> 틀렸다. 노드 형으로 심볼메이크에 넣어야 같은 형태가 되어 심볼메이킹 처리에 수정이 없다.
              ㄴ 타입
              ㄴ IDENT
...
(인자가 void인 경우는 이전과 같이

이다.)

함수 선언의 인자 처리를 비우는 것은 불법이므로, param_list의 son이 null일 수는 없다.
-> 별다른 검증 없이 param_list의 son을 이용할 수 있다.

인자 타입을 심볼에 기록하고 체크하는 것은 이후 pass에서 진행할 것.
일단은 타입체킹 없이!

────────────────────────────────

심볼 저장: 함수의 경우는, 타입 트리를 좀 바꿔보기?

리턴 타입, 인자 등등의 타입을 각각의 개체별로, sem_type 등으로 감싸야 하지 않을까?
-> 타입의 brother를 가진 인자에 대비,
...
현재 첫 선언 시 타입트리 구하는 방식 사용 -> 현재 방식도 새로 만든 함수로 대체.
